...
def create_pitch_3d():
    ...
    z=[-1, -1, -1, -1],  # All points are at ground level
...
def create_3d_plot(df, event_dict, chosen_timestamp, displayed_event, voronoi,fig):
    ...
    if row['type'] == 'Carry':
        ...
        if z2 > 0:  # Only create curves for shots where z2 is greater than 0
            x_curve, y_curve, z_curve = generate_smooth_curve(row['location'][0], row['shot_end_location'][0], 80-row['location'][1], 80-row['shot_end_location'][1], zstart,z2,foot,team)
...
def generate_smooth_curve(x_start, x_end, y_start, y_end, z_start, z_end, foot, team):
    t = np.linspace(0, 1, num=100)  # Parameter t from 0 to 1
    x_curve = (1 - t) * x_start + t * x_end
    y_curve = (1 - t) * y_start + t * y_end
...
def shot_freeze_frame_3d(fig,shot_df, tag, keeper_cone=True):
    ...
    # Add the shot location
        ...
        if z2 > 0:  # Only create curves for shots where z2 is greater than 0
            x_curve, y_curve, z_curve = generate_smooth_curve(pitch_width-shot['location'][0], pitch_width-shot['shot_end_location'][0], shot['location'][1], shot['shot_end_location'][1], zstart,z2,foot,team)
    ...
    # Add the shot location
        ...
        if z2 > 0:  # Only create curves for shots where z2 is greater than 0
            x_curve, y_curve, z_curve = generate_smooth_curve(shot['location'][0], shot['shot_end_location'][0], 80-shot['location'][1], 80-shot['shot_end_location'][1], zstart,z2,foot,team)
...
if trythreesixty:
    ...
    def pressure_map_3d(fig):
        ...
        #         x=x[i],
        #         y=y[i],
        #         z=[0],  # Assuming carries occur at the same z-coordinate
        #         mode='markers',
        #         marker=dict(color=color, size=12, symbol='circle', opacity=0.5),
        #         name='Pressure Points',
        #         hoverinfo='text',
    ...
    def shot_map_3d(fig):
        ...
        for i in range(len(x1)):
            ...
            if menu_team == team_2:
                ...
                if z2[i] > 0:  # Only create curves for shots where z2 is greater than 0
                    x_curve, y_curve, z_curve = generate_smooth_curve(x1[i], x2[i], y1[i], y2[i], zstart,z2[i],foot,team)
